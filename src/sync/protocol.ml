...
module Negotiation = struct
  ...
  let map ~fn = function
    | ACK uid -> ACK (fn uid)
    | ACK_continue uid -> ACK_continue (fn uid)
    | ACK_ready uid -> ACK_ready (fn uid)
    | ACK_common uid -> ACK_common (fn uid)
    | NAK -> NAK
end
...
module Shallow = struct
  ...
  let map ~fn = function
    | Shallow v -> Shallow (fn v)
    | Unshallow v -> Unshallow (fn v)
end
...
module Status = struct
  ...
  let map ~fn {result; commands} =
    let commands =
      let fold = function
        | `FF ref -> `FF (fn ref)
        | `OK ref -> `OK (fn ref)
        | `ER (ref, err) -> `ER (fn ref, err)
      in
...
module Decoder = struct
  ...
  let decode_advertised_refs decoder =
      ...
      let rec go refs decoder =
        ...
        if String.Sub.is_empty v then (
          ...
          | Some (uid, reference) ->
            ...
            junk_pkt decoder; prompt_pkt k decoder
          | None ->
    ...
    let decode_no_ref ~version v decoder =
      ...
      | Some (_, capabilities) ->
        ...
        let capabilities =
          List.map (Capability.of_string <.> String.Sub.to_string) capabilities
        in
        junk_pkt decoder;
        let k decoder =
          ...
          if String.Sub.length pkt = 0 then (
            ...
            return
              ...
              decoder)
          else fail decoder (`Invalid_advertised_ref (String.Sub.to_string pkt))
        in
        prompt_pkt k decoder
    ...
    let decode_refs ?(version = 1) decoder =
      ...
      match String.Sub.cut ~sep:v_space v with
      | Some (uid, _) ->
        if String.Sub.for_all is_zero uid then decode_no_ref ~version v decoder
  ...
  let decode_result decoder =
    let k decoder =
      ...
      if String.Sub.equal_bytes v v_nak then (
        junk_pkt decoder; return Result.NAK decoder)
      else
        match String.Sub.cut ~sep:v_space v with
        | Some (_, common) ->
  ...
  let decode_packet ~trim decoder =
    let k decoder =
      ...
      junk_pkt decoder; return r decoder
    in
    prompt_pkt k decoder
  ...
  let decode_have decoder =
    let rec go haves decoder =
      ...
      if Sub.is_empty v then (
        ...
        | Some (have, new_have) ->
          let haves =
            if Sub.equal_bytes have v_have then Sub.to_string new_have :: haves
            else haves
          in
  ...
  let decode_pack ?(side_band = false) ~push_stdout ~push_stderr decoder =
    let with_side_band decoder =
      ...
      | Some '\002' ->
        let tail =
          String.Sub.to_string (String.Sub.tail v)
          (* copy *)
        in
        push_stdout tail; junk_pkt decoder; return `Stdout decoder
      | Some '\003' ->
        let tail =
          String.Sub.to_string (String.Sub.tail v)
          (* copy *)
        in
        push_stderr tail; junk_pkt decoder; return `Stderr decoder
  ...
  let decode_negotiation decoder =
    let rec k decoder =
      ...
      if String.Sub.equal_bytes pkt v_nak then (
        ...
        | _ -> fail decoder (`Invalid_ack (String.Sub.to_string pkt))
      else (junk_pkt decoder; prompt_pkt k decoder)
    in
    prompt_pkt k decoder
  ...
  let decode_status decoder =
    ...
    let commands res decoder =
      let rec go acc decoder =
        ...
        | Ok None -> return {Status.result= res; commands= List.rev acc} decoder
  ...
  let decode_commands decoder =
    let rec rest_commands ({Commands.commands= first, rest; _} as res) decoder =
      let pkt = peek_pkt decoder in
      match String.Sub.cuts ~sep:v_space pkt with
      | [oid0; oid1; reference] ->
        let reference = String.Sub.to_string reference in
        let commands =
          ...
          | false, false ->
            ( first,
              Commands.Update
                (String.Sub.to_string oid0, String.Sub.to_string oid1, reference)
              :: rest )
          | _ -> assert false
        in
        junk_pkt decoder;
    ...
    let first_command decoder =
      ...
      | Some (cmd, capabilities) -> (
        ...
        | [oid0; oid1; reference] ->
          ...
          in
          junk_pkt decoder;
          prompt_pkt (rest_commands {Commands.capabilities; commands}) decoder
        | _ -> fail decoder (`Invalid_command (String.Sub.to_string cmd)))
      | None -> junk_pkt decoder; return None decoder
    in
...
module Encoder = struct
  ...
  let kflush encoder = write encoder "0000"; flush kdone encoder
  let encode_flush encoder = kflush encoder

  let encode_proto_request
      encoder {Proto_request.path; host; version; request_command} =
  ...
  let encode_want
    ...
    let shallows encoder =
      let shallow hash encoder =
        write encoder "shallow"; write_space encoder; write encoder hash
      in
    ...
    let others encoder =
      let want hash encoder =
        write encoder "want"; write_space encoder; write encoder hash
      in
    ...
    let first encoder =
      ...
      if List.length capabilities > 0 then (
        write_space encoder; go capabilities);
  ...
  let encode_done encoder =
    let k encoder = write encoder "done"; write_new_line encoder in
    delayed_write_pkt k kdone encoder
  ...
  let encode_commands encoder {Commands.capabilities; commands= first, others} =
    let others encoder =
  ...
  let encode_advertised_refs encoder advertised_refs =
    let encode_shallows shallows encoder =
      let encode_shallow shallow encoder =
        write encoder "shallow"; write_space encoder; write encoder shallow
      in
      let rec go shallows encoder =
        match shallows with
        | [] -> kflush encoder
  ...
  let encode_acks encoder acks =
    ...
    let write_ack ack encoder =
      let write_ack uid suffix =
        ...
        (match suffix with
        | None -> ()
        | Some s -> write_space encoder; write encoder s);
        write_new_line encoder
      in
      match ack with
