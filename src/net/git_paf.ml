...
let git_paf_scheme : [ `HTTP | `HTTPS ] Mimic.value =
  Mimic.make ~name:"git-paf-scheme"
  ...
  let rec go max uri =
    ...
    if status_code / 100 = 3 then (
      ...
      | Some uri' when Hashtbl.mem tbl uri' || max = 0 ->
        Log.warn (fun m ->
            m
              "We found a cycle (%a) or reach the limit of redirections (max: \
               %d)."
              Uri.pp uri' max);
...
let call
  ...
  let req =
    H1.Request.create ~headers (meth :> H1.Method.t) (Uri.path_and_query uri)
  in
  ...
  | Ok flow -> (
    ...
    | `Error (`Invalid_response_body_length resp) ->
      Mimic.close flow >>= fun () -> raise (Invalid_response_body_length resp)
