...
module TCP = struct
  ...
  let shutdown fd mode =
    ...
    in
    Lwt_unix.shutdown fd m; Lwt.return_unit
...
module A = Git_net.TCP.Make (TCP) (Happy_eyeballs)
module B = Git_net.SSH.Make (TCP) (Happy_eyeballs)
module C = Git_net.HTTP.Make (TCP) (Happy_eyeballs)
...
let ctx happy_eyeballs =
  ...
  let ctx =
    Mimic.fold fifo_endpoint
      Mimic.Fun.
        [
          req Git_store.Endpoint.git_transmission;
          req Git_store.Endpoint.git_scheme; req Git_store.Endpoint.git_hostname;
        ]
      ~k:k1 ctx
  in
  let ctx =
